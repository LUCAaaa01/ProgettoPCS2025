...
namespace vertex{
    ...
    */
    unsigned int add(PolyhedronCollection& p_coll, const Eigen::Vector3d new_point, bool checkpointexists = true);

    // rimuovo il punto, se esiste e non viene usato in nessun segmento o faccia
    bool remove(PolyhedronCollection& p_coll, unsigned int point_id); // --- non so se ha senso
